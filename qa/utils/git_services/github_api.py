...
class GithubAPI:
    ...
    async def process_result(self, result: dict, results: list, lang: str = "en") -> None:
        ...
        contributors = await self.get_contributors(full_name)
        repo_readme_content = await self.get_readme_content(full_name)
    ...
    async def search_repositories(self, query, repos: list, lang: str = "en"):
        ...
        try:
            ...
            for repo_info in data.get("items", []):
                _t = threading.Thread(
                    target=asyncio.run,
                    args=(self.process_result(repo_info, repos, lang),),
                    daemon=True,
                )
                threads.append(_t)
                _t.start()
    ...
    async def get_contributors(self, full_name):
